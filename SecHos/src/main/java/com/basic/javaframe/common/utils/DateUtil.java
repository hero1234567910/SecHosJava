...
public class DateUtil {
	 ...
	 * 
	 * @Title: changeTimeStr 
	 * @Description: 时间戳字符串转时间字符串 
	 * @param @param dateStr
	 * @param @return    设定文件 
	 * @return String    返回类型 
	 * @throws
	 */
	public static String changeTimeStr(String dateStr) {
		if (StringUtil.isEmpty(dateStr) == true) {
			return "";
		}
		return SDF_Y_M_D.format(new Date(Long.valueOf(dateStr)));
	}
...
//			Date dt2 = SDF_Y_M_D.parse(DATE2);
	...
	public static Date changeDate(Date date) {
		String dateStr = SDF_YMDHMS.format(date);
		Date date2 = null;
		try {
